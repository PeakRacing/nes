/*
 * Copyright PeakRacing
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#include "nes.h"

/* https://www.nesdev.org/wiki/INES_Mapper_229
 * BMC 31-in-1 — bank selection via address bits.
 * Write $8000-$FFFF: address[4:0] = bank index
 *   if address[0]=0: 16KB mode (bank at $8000, bank+1 at $C000)
 *   if address[0]=1: 32KB mode (bank>>1 fills both halves)
 * CHR bank = address[4:0].
 * Mirroring: address[5] (0=V, 1=H).
 */

static void nes_mapper_init(nes_t* nes) {
    nes_load_prgrom_16k(nes, 0, 0);
    nes_load_prgrom_16k(nes, 2, 1);
    if (nes->nes_rom.chr_rom_size > 0) {
        nes_load_chrrom_8k(nes, 0, 0);
    }
}

static void nes_mapper_write(nes_t* nes, uint16_t address, uint8_t data) {
    (void)data;
    uint8_t bank = (uint8_t)(address & 0x1Fu);
    if (address & 0x01u) {
        /* 32KB mode */
        nes_load_prgrom_32k(nes, 0, (uint16_t)(bank >> 1));
    } else {
        /* 16KB mode */
        nes_load_prgrom_16k(nes, 0, bank);
        nes_load_prgrom_16k(nes, 2, (uint8_t)(bank + 1u));
    }
    if (nes->nes_rom.chr_rom_size > 0) {
        nes_load_chrrom_8k(nes, 0, bank);
    }
    if (nes->nes_rom.four_screen == 0) {
        nes_ppu_screen_mirrors(nes, (address & 0x20u) ? NES_MIRROR_HORIZONTAL : NES_MIRROR_VERTICAL);
    }
}

int nes_mapper229_init(nes_t* nes) {
    nes->nes_mapper.mapper_init  = nes_mapper_init;
    nes->nes_mapper.mapper_write = nes_mapper_write;
    return NES_OK;
}
