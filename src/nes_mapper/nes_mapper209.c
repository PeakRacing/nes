...
/* https://www.nesdev.org/wiki/INES_Mapper_209
 * Mapper 209 — J.Y. Company (variant, same as mapper 90 with CPU-cycle IRQ).
 * The register layout and banking are identical to mapper 90 (J.Y. Company).
 * The difference is the IRQ counts CPU cycles rather than scanlines.
 * For MCU targets, scanline-based IRQ provides adequate compatibility.
 */
...
static void mapper209_update_prg(nes_t* nes) {
    mapper209_t* r = (mapper209_t*)nes->nes_mapper.mapper_register;
...
static void mapper209_update_chr(nes_t* nes) {
    mapper209_t* r = (mapper209_t*)nes->nes_mapper.mapper_register;
...
static const nes_mirror_type_t jy_mirror209[4] = {
    NES_MIRROR_VERTICAL,
...
static void nes_mapper_write(nes_t* nes, uint16_t address, uint8_t data) {
    ...
    case 0x8000u:
        ...
        mapper209_update_prg(nes);
        break;
    case 0x9000u:
        r->chr[address & 0x07u] = data;
        mapper209_update_chr(nes);
        break;
    case 0xA000u:
    ...
    case 0xF000u:
        switch (address & 0x03u) {
        case 0u: r->prg_mode = data & 0x03u; mapper209_update_prg(nes); break;
        case 1u: r->chr_mode = data & 0x03u; mapper209_update_chr(nes); break;
        case 2u:
            r->mirror = data & 0x03u;
            if (nes->nes_rom.four_screen == 0)
                nes_ppu_screen_mirrors(nes, jy_mirror209[r->mirror]);
            break;
...
static void nes_mapper_init(nes_t* nes) {
    if (nes->nes_mapper.mapper_register == NULL) {
        nes->nes_mapper.mapper_register = nes_malloc(sizeof(mapper209_t));
        if (nes->nes_mapper.mapper_register == NULL) return;
    }
    mapper209_t* r = (mapper209_t*)nes->nes_mapper.mapper_register;
    nes_memset(r, 0, sizeof(mapper209_t));
