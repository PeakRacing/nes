 ...
 * BMC 20-in-1 — bank select via address bits.
 * Write $8000-$FFFF:
 *   address[4:0] = bank index
 *   address[5]   = mirroring (0=V, 1=H)
 * If address[4]=1: 32KB mode; else 16KB mode (both halves same bank).
 */
...
static void nes_mapper_write(nes_t* nes, uint16_t address, uint8_t data) {
    (void)data;
    uint8_t bank = (uint8_t)(address & 0x1Eu);
    if (address & 0x10u) {
        nes_load_prgrom_32k(nes, 0, (uint16_t)(bank >> 2));
    } else {
